...
if __name__ == '__main__':
	...
	if Q7:
		...
		df_pos_mean = df.groupby('syntactic_category').mean() # Variable is called syntactic_category here (as opposed to pos_category in Expt 1)
		# suffix _mean to column names
		df_pos_mean.columns = [f'{col}_mean' for col in df_pos_mean.columns]
		# Also get std; merge for saving
		df_pos_std = df.groupby('syntactic_category').std()
		...
		if save:
			...
			# Order: ADJ, NOUN, VERB
			df_pos_grouped = df_pos_grouped.reindex(['ADJ', 'NOUN', 'VERB'])
			...
			plt.ylim(0, 1)
			plt.title('Experiment 2: Mean memorability accuracy per POS category')
			plt.tight_layout()
		...
		# 								 predictors=['syntactic_category'], save_subfolder=save_subfolder,
		# 								 demean_x=False, # categorical
		# 								 demean_y=True,
		# 								 formula='C(syntactic_category)')

		df_baseline_pos = get_cv_score(df=df, acc1=acc1, acc2=acc2, save=save, result_dir=RESULTDIR,
										 model_name='baseline_pos_category',
										 predictors=['num_meanings_human', 'num_synonyms_human', 'syntactic_category'],
									     save_subfolder=save_subfolder,
										 formula='num_meanings_human + num_synonyms_human + C(syntactic_category)')
		...
		m_pos = smf.ols('acc_demean ~ C(syntactic_category)',
								   data=df).fit()

		m_baseline_pos = smf.ols('acc_demean ~ num_meanings_human_demean + num_synonyms_human_demean + C(syntactic_category)',
								   data=df).fit()

		m_pos_meanings = smf.ols('acc_demean ~ num_meanings_human_demean + C(syntactic_category)',
								   data=df).fit()
		m_pos_synonyms = smf.ols('acc_demean ~ num_synonyms_human_demean + C(syntactic_category)',
								   data=df).fit()
		...
		comp_baseline_human_pos[
			'model_add_predictor'] = 'acc_demean ~ num_meanings_human_demean + num_synonyms_human_demean + C(syntactic_category)'

		# Compare Mem ~ pos WITH Mem ~ meanings + pos
		comp_pos_meanings = sm.stats.anova_lm(m_pos, m_pos_meanings)
		comp_pos_meanings['model'] = 'acc_demean ~ syntactic_category'
		comp_pos_meanings[
			'model_add_predictor'] = 'acc_demean ~ syntactic_category + num_meanings_human_demean'

		# Compare Mem ~ pos WITH Mem ~ synonyms + pos
		comp_pos_synonyms = sm.stats.anova_lm(m_pos, m_pos_synonyms)
		comp_pos_synonyms['model'] = 'acc_demean ~ syntactic_category'
		comp_pos_synonyms[
			'model_add_predictor'] = 'acc_demean ~ syntactic_category + num_synonyms_human_demean'
